...
// 4. VISTAS INTERNAS DEL MÓDULO
function setupSurveyViews() {
    const activateControl = (control, event) => {
        if (!control) return;
        event.preventDefault();
        showSurveyView(control.dataset.surveyViewTarget);
    };

    document.addEventListener("click", (event) => {
        const control = event.target.closest("[data-survey-view-target]");
        if (control) {
            activateControl(control, event);
        }
    });

    document.querySelectorAll(".survey-view-button").forEach((control) => {
        control.addEventListener("pointerdown", (event) => {
            activateControl(control, event);
        });
    });

    showSurveyView("capture", { silent: true });
}

function showSurveyView(view, options = {}) {
    const nextView = view === "knowledge" ? "knowledge" : "capture";
    const captureView = document.getElementById("survey-capture-view");
    const knowledgeView = document.getElementById("survey-knowledge");
    ...
    activeSurveyView = nextView;

    if (captureView) {
        captureView.classList.toggle("hidden", nextView !== "capture");
    }
    if (knowledgeView) {
        knowledgeView.classList.toggle("hidden", nextView !== "knowledge");
    }
    ...
    if (nextView === "capture") {
        ...
        if (sectionKicker) sectionKicker.textContent = "Consola de campo";
        if (sectionTitle) sectionTitle.textContent = "Levantamiento profesional LAN";
    } else {
        modeSelection?.classList.add("hidden");
        workspace?.classList.add("hidden");
        if (sectionKicker) sectionKicker.textContent = "Referencia técnica";
        if (sectionTitle) sectionTitle.textContent = "Base normativa del levantamiento";
    }

    document.querySelectorAll(".survey-view-button").forEach((button) => {
        const isActive = button.dataset.surveyViewTarget === nextView;
...
function hydrateSurveyForm() {
    const fields = {
        "building-name": surveyData.buildingName,
        "building-address": surveyData.buildingAddress,
        "building-use": surveyData.buildingUse,
        "report-purpose": surveyData.reportPurpose,
        "report-scope": surveyData.reportScope,
        "report-background": surveyData.reportBackground,
        "report-findings": surveyData.reportFindings,
        "report-wifi": surveyData.reportWifi,
        "report-security": surveyData.reportSecurity
    };
...
function mergeByIdOrName(existingItems = [], incomingItems = [], childrenMap = {}) {
    const mergedItems = [...existingItems];

    incomingItems.forEach((incomingItem) => {
        const matchIndex = mergedItems.findIndex((item) => {
            if (incomingItem.id && item.id === incomingItem.id) return true;
            if (incomingItem.name && item.name && item.name === incomingItem.name) return true;
            return false;
        });
        ...
        const currentItem = mergedItems[matchIndex];
        const nextItem = { ...currentItem, ...incomingItem };

        Object.entries(childrenMap).forEach(([childKey, nestedChildrenMap]) => {
            nextItem[childKey] = mergeByIdOrName(
                Array.isArray(currentItem[childKey]) ? currentItem[childKey] : [],
                Array.isArray(incomingItem[childKey]) ? incomingItem[childKey] : [],
                nestedChildrenMap
            );
        });
...
function mergeSurveyProject(currentData, incomingData) {
    const hasCurrentRecords = Boolean(currentData.mode && Array.isArray(currentData.floors) && currentData.floors.length);
    if (!hasCurrentRecords) {
        return {
            ...currentData,
            ...incomingData,
            floors: Array.isArray(incomingData.floors) ? incomingData.floors : []
        };
    }
    ...
    return {
        ...currentData,
        mode: currentData.mode || incomingData.mode || "",
        buildingName: incomingData.buildingName || currentData.buildingName || "",
        buildingAddress: incomingData.buildingAddress || currentData.buildingAddress || "",
        buildingUse: incomingData.buildingUse || currentData.buildingUse || "",
        reportPurpose: incomingData.reportPurpose || currentData.reportPurpose || "",
        reportScope: incomingData.reportScope || currentData.reportScope || "",
        reportBackground: incomingData.reportBackground || currentData.reportBackground || "",
        reportFindings: incomingData.reportFindings || currentData.reportFindings || "",
        reportWifi: incomingData.reportWifi || currentData.reportWifi || "",
        reportSecurity: incomingData.reportSecurity || currentData.reportSecurity || "",
        floors: mergeByIdOrName(currentData.floors || [], incomingData.floors || [], hierarchyMap)
    };
}
    ...
    reader.onload = () => {
        try {
            ...
            if (data.mode && Array.isArray(data.floors)) {
                const wasMerged = Boolean(surveyData.mode && surveyData.floors.length);
                surveyData = mergeSurveyProject(surveyData, data);
                hydrateSurveyForm();

                initSurvey(surveyData.mode);
                saveSurveyToLocalStorage();
                showToast("📂 Cargado", wasMerged ? "JSON integrado sin eliminar registros existentes." : "Proyecto importado y renderizado con éxito.", "success");
            } else {
